...
$tablesDir     = $app->path('tables:');
$tablesI18nDir = 'tables:i18n';
...
$directory = new RecursiveDirectoryIterator($tablesDir);
...
if (count($strings)) {
    ...
    if ($app->path("{$tablesI18nDir}/{$lang}.php")) {
        $langfile = include($app->path("{$tablesI18nDir}/{$lang}.php"));
    ...
    $app->helper('fs')->write("{$tablesI18nDir}/{$lang}.php", '<?php return '.varexport($strings, true).';');
}

CLI::writeln("Done! Language file created: {$tablesI18nDir}/{$lang}.php", true);
